"""
Breathing Rate Detection from CSI Data
source file path parameter needs to be changed manually 
bpm estimation will be stored in a CSV file
and plots (raw CSI, smoothed CSI, ACF) will be saved in a specified directory.
"""
import matplotlib
matplotlib.use('Agg')
import pandas as pd
...
def load_csi_data(file_path):
    """
    Loads CSI data from a CSV file.

    Args:
        file_path (str): The path to the CSV file.

    Returns:
        pandas.DataFrame: A DataFrame containing the CSI data with 'timestamp' and 'data' columns,
                          or None if loading fails or required columns are missing.
    """
    try:
...
def extract_amplitudes_all_subcarriers(csi_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if valid_amplitudes.empty:
        ...
        return pd.DataFrame()
    #  --- CHECK: check lengths of valid amplitudes ---
    # valid_lengths = valid_amplitudes.apply(len)
    # if not valid_lengths.empty and valid_lengths.nunique() > 1:
    #     print(f"Warning: Number of extracted subcarriers varies across valid rows.")
    #     print(f"Unique lengths found: {valid_lengths.unique()}")
    #     print(f"Count of each length:\n{valid_lengths.value_counts()}")

    # RESULTS: Warning: Number of extracted subcarriers varies across valid rows.
        # Unique lengths found: [117  53 128]
...
def save_estimated_bpms_to_csv(estimated_results: list, output_dir: str = "./results"):
    ...
    csv_filename = "estimated_bpm_results.csv" 
...
def estimate_breathing_rate_windowed(csi_df: pd.DataFrame, sampling_rate: int = 100, window_duration_sec: int = 15, step_duration_sec: int = 1, output_plots_dir: str = "./window_plots_results_2"):
    """
    ...
    for start_index in range(0, len(processed_df_for_plots) - window_size_samples + 1, step_size_samples):
        ...
        breathing_cutoff_freq_lowpass_plot = 1.5
        filter_order_plot = 5
        ...
        if len(window_amplitudes_df) < min_signal_length_for_acf or window_amplitudes_df.shape[1] == 0:
             ...
             current_window_plot_data = {
                'window_start_timestamp': window_start_timestamp,
                'window_end_timestamp': window_end_timestamp,
                'estimated_bpm': None,
                'raw_signal': raw_window_signal_for_plot.copy(),
                'smoothed_signal': smoothed_window_signal_for_plot.copy() if len(smoothed_window_signal_for_plot) > 0 else np.array([]),
                'acf_values': np.array([]), 
                'selected_peak_lag_samples': None
             }
             plot_window_data(current_window_plot_data, sampling_rate=sampling_rate, output_dir=output_plots_dir)
             continue
        ...
        if len(combined_acf) < 2 or np.sum(np.abs(combined_acf)) == 0: 
             ...
             min_peak_distance_for_breathing = int(sampling_rate * 1.2) 
             if len(smoothed_combined_acf[1:]) == 0:
                  ...
                  try:
                      peaks, properties = find_peaks(smoothed_combined_acf[1:], distance=min_peak_distance_for_breathing, prominence=0.12) 
        ...
        current_window_plot_data = {
            'window_start_timestamp': window_start_timestamp,
            'window_end_timestamp': window_end_timestamp,
            'estimated_bpm': estimated_bpm_rounded, 
            'raw_signal': raw_window_signal_for_plot.copy() if raw_window_signal_for_plot is not None else np.array([]), 
            'smoothed_signal': smoothed_window_signal_for_plot.copy() if smoothed_window_signal_for_plot is not None and len(smoothed_window_signal_for_plot) > 0 else np.array([]), 
            'acf_values': acf_values_for_plot.copy() if acf_values_for_plot is not None and len(acf_values_for_plot) > 0 else np.array([]), 
            'selected_peak_lag_samples': selected_peak_lag_samples
        }
        plot_window_data(current_window_plot_data, sampling_rate=sampling_rate, output_dir=output_plots_dir)
